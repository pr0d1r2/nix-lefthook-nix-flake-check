...
| status | id | goal |
|---|---|---|
| `.` | T9 | Create `nix-flake-check-validate-timeout.sh` that validates its argument is a positive integer (exit 0 valid, exit 1 otherwise) with 1-to-1 bats test `tests/unit/nix-flake-check-validate-timeout.bats` covering positive int, non-numeric string, zero, negative, empty (§B.3, §V.2, §V.5) |
